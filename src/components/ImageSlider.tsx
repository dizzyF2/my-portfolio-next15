...
export default function ImageSlider({ images, altText }: ImageSliderProps) {
    ...
    return (
        ...
        {!isZoomed && (
            <div
                ref={sliderRef}
                className="relative overflow-hidden w-full h-64 tablet:h-80 rounded-lg group select-none"
                onTouchStart={onTouchStart}
                onTouchMove={onTouchMove}
                onTouchEnd={onTouchEnd}
            >
            {/* Main image */}
            <div onClick={handleZoom} className="relative h-full w-full">
                <Image
                    src={imageArray[currentIndex] || "/placeholder.svg"}
                    alt={altText}
                    fill
                    className="absolute rounded-lg object-cover size-full transition-transform duration-500 pointer-events-none"
                    draggable={false}
                />
            </div>
            ...
            {imageArray.length > 1 && (
                <>
                    <button
                        onClick={prevSlide}
                        className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 p-2 rounded-full cursor-pointer focus:outline-none z-10"
                    ...
                    <button
                        onClick={nextSlide}
                        className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 p-2 rounded-full cursor-pointer focus:outline-none z-10"
            ...
            {/* Zoom button */}
                <button
                    onClick={handleZoom}
                    className="absolute bottom-2 right-2 bg-black/50 hover:bg-black/70 p-2 rounded-full cursor-pointer focus:outline-none z-10"
            ...
            {/* Image counter */}
                {imageArray.length > 1 && (
                    <div className="absolute bottom-2 left-2 bg-black/50 px-2 py-1 rounded text-xs text-white z-10">
            ...
            >
                ...
                {/* ZOOM SLIDE ARROWS */}
                    ...
                    <>
                        <button
                            onClick={prevSlide}
                            className="absolute left-2 tablet:left-3 laptop:left-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 p-3 rounded-full cursor-pointer focus:outline-none z-10"
                            aria-label="Previous image"
                        >
                        ...
                        <button
                            onClick={nextSlide}
                            className="absolute right-2 tablet:right-3 laptop:right-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 p-3 rounded-full cursor-pointer focus:outline-none z-10"
                            aria-label="Next image"
                        >
